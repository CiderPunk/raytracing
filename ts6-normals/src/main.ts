...
export class Main{
  ...
  hit_sphere(center:Point3,radius:number, r:Ray):number {
  ...
  ray_color(ray:Ray):Color{
    const t = this.hit_sphere(new Vec3(0,0,-1), 0.5, ray);
    if (t > 0.0) {
      const N = ray.at(t).subInPlace(new Vec3(0,0,-1)).unit();
      return new Color(N.x+1, N.y+1, N.z+1).scaleInPlace(0.5) as Color;
    }
